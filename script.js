...
function updateGalaxyLyricConstellations(current, elapsedTime) {
    ...
    // --- EVENTO GLOBAL DE SUPERNOVA ---
    const SUPERNOVA_START = 148.8;   // 0.8s después de la trompeta, inicia la succión
    const SUPERNOVA_EXPLODE = 149.8; // Detonación masiva

    if (current < SUPERNOVA_START) {
        // ============================================================
        // LÓGICA NORMAL: formación por frase + disolución individual
        // ============================================================
        galaxyLyricConstellations.forEach((constellation) => {
            ...
            for (let i = 0; i < data.count; i++) {
                ...
                const releaseClamped = clamp01(releaseRaw);

                // Color por defecto: el dorado pálido original (#fff4ba)
                let cr = 1.0, cg = 0.9569, cb = 0.7294;

                if (releaseClamped <= 0) {
                    // --- Formación / flotación libre de la letra ---
                    const freeFloat =
                        Math.sin(elapsedTime * 0.50 + theta) *
                        0.026 *
                        (1 - presence);

                    const freeDriftX =
                        Math.cos(elapsedTime * 0.33 + theta * 1.7) *
                        0.018 *
                        (1 - presence);

                    const freeDriftZ =
                        Math.sin(elapsedTime * 0.29 + theta * 1.3) *
                        0.018 *
                        (1 - presence);

                    const letterBreath =
                        Math.sin(elapsedTime * 1.65 + theta) *
                        0.0045 *
                        presence;
                ...
                } else if (releaseClamped <= 0.25) {
                    // --- FASE 1: COLAPSO — succión violenta hacia (0,0,0) ---
                    const collapseT = releaseClamped / 0.25;
                    const pull = collapseT * collapseT * collapseT; // ease-in cúbico

                    positions[j]     = tx * (1 - pull);
                    positions[j + 1] = ty * (1 - pull);
                    positions[j + 2] = tz * (1 - pull);

                } else {
                    // --- FASE 2: EXPLOSIÓN esférica + FASE 3: DANZA CUÁNTICA ---
                    const t2 = (releaseClamped - 0.25) / 0.75;

                    // Vector de escape esférico determinístico (sin new Vector3)
                    const phi = s * Math.PI;
                    ...
                    // Expansión rápida al inicio (gran velocidad), se frena luego
                    const burstRadius = Math.sqrt(t2) * (4.0 + s * 3.0);

                    positions[j]     = dirX * burstRadius;
                    positions[j + 1] = dirY * burstRadius;
                    positions[j + 2] = dirZ * burstRadius;

                    // Danza caótica: entra progresivamente entre 0.3 y 1.0
                    const danceT = clamp01((releaseClamped - 0.3) / 0.7);

                    if (danceT > 0) {
                        const chaosX = Math.sin(elapsedTime * 3.0 + theta * 4.0 + s * 5.0) * 0.4 * danceT;
                        const chaosY = Math.cos(elapsedTime * 2.6 + theta * 3.3 + s * 6.0) * 0.4 * danceT;
                        const chaosZ = Math.sin(elapsedTime * 3.3 + theta * 2.1 + s * 4.0) * 0.4 * danceT;

                        positions[j]     += chaosX;
                        positions[j + 1] += chaosY;
                        positions[j + 2] += chaosZ;

                        // Parpadeo intenso dorado/blanco, como neutrinos libres
                        const flicker = 0.5 + 0.5 * Math.sin(elapsedTime * 8.0 + theta * 6.0 + s * 10.0);
                        const glow = danceT * (0.6 + flicker * 0.9);

                        cr = 1.0 + glow * 0.8;
                        cg = 0.85 + flicker * 0.15 + glow * 0.5;
                        cb = 0.2 + flicker * 0.6 + glow * 0.3;
                    }
                }
        ...
        // ============================================================
        // OVERRIDE TOTAL: EVENTO GLOBAL DE SUPERNOVA
        // ============================================================
        galaxyLyricConstellations.forEach((constellation) => {
            const data = constellation.userData;
            const positions = constellation.geometry.attributes.position.array;
            ...
            // Todas las constelaciones de la canción reaparecen para la explosión
            constellation.visible = true;
            ...
            if (current < SUPERNOVA_EXPLODE) {
                // --- FASE DE SUCCIÓN: colapso exponencial hacia (0,0,0) ---
                const collapseT = clamp01(
                    (current - SUPERNOVA_START) / (SUPERNOVA_EXPLODE - SUPERNOVA_START)
                );
                ...
                for (let i = 0; i < data.count; i++) {
                    ...
                    // Color progresivo hacia blanco/dorado hirviente
                    colors[j]     = 1.0 * (1 - pull) + 1.0 * pull;
                    colors[j + 1] = 0.9569 * (1 - pull) + 0.9 * pull;
                    colors[j + 2] = 0.7294 * (1 - pull) + 0.5 * pull;
                }

            } else {
                // --- FASE DE EXPLOSIÓN Y DANZA ---
                ...
                for (let i = 0; i < data.count; i++) {
                    ...
                    // Vector de escape esférico pseudo-aleatorio determinístico por índice
                    ...
                    // Expansión rápida basada en explodeT
                    ...
                    // Rebote/baile caótico, como luciérnagas junto a los neutrinos
                    ...
                    // Dorado/blanco intenso y parpadeante
