...
function updateGalaxyLyricConstellations(current, elapsedTime) {
    ...
    galaxyLyricConstellations.forEach((constellation) => {
        ...
        for (let i = 0; i < data.count; i++) {
            ...
            if (releaseClamped <= 0) {
                // --- Sin cambios: formación / flotación libre de la letra ---
                const freeFloat =
                ...
                const pull = collapseT * collapseT * collapseT; // ease-in cúbico: arranca y se dispara
...
function initGalaxy() {
    ...
    const neutrinoSize = isMobile ? (isHighEndMobile ? 26.0 : 21.0) : 29.0;
    ...
    const neutrinoMat = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                vAlpha = 0.5 + 0.85 * sin(uTime * 3.0 + position.x * 16.0 + position.y * 11.0);
