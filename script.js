...
function animateBoids() {
    ...
    for (let boid of boids) {
        ...
        if (numNeighbors > 0) {
            ...
            if (boid.isGlitch) {
                boid.vx += (boid.x - centerX) * centeringFactor * 8; 
                boid.vy += (boid.y - centerY) * centeringFactor * 8;
...
function createUnifiedGalaxy(parameters, isMobile, isHighEndMobile) {
    ...
    // Cantidades optimizadas para mantener la forma sin saturar
    const starCount = isMobile ? (isHighEndMobile ? 35000 : 20000) : 80000;
    const dustCount = isMobile ? (isHighEndMobile ? 8000 : 5000) : 20000;
    ...
    const dustLayer = buildLayer(dustCount, isMobile ? 0.07 : 0.1, dustTexture, 0.18, true);
    const starsLayer = buildLayer(starCount, isMobile ? 0.016 : 0.022, starTexture, 0.9, false);
...
function initGalaxy() {
    ...
    // Un ShaderPass simple (ShaderPass.js ya está cargado, misma librería
    // que usa el bloom, no se agrega nada nuevo). Oscurece un poco los
    // bordes de la pantalla y le suma un ruido finito muy leve, como el
    // grano de una foto de larga exposición real.
    const vignetteGrainPass = new THREE.ShaderPass({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                // Viñeta: oscurece suavemente hacia las esquinas
                vec2 centered = vUv - 0.5;
                float vignette = 1.0 - dot(centered, centered) * uVignetteStrength;
                color.rgb *= clamp(vignette, 0.0, 1.0);

                // Grano: ruido fino que cambia cada frame, muy sutil
    ...
    // --- SISTEMA DE NEUTRINOS (LUCIÉRNAGAS CUÁNTICAS) ---
    const neutrinoGeo = new THREE.BufferGeometry();
    const neutrinoCount = isMobile ? (isHighEndMobile ? 400 : 250) : 800; // Menos partículas para más elegancia
    ...
    const colorA = new THREE.Color('#ffdd77'); // Dorado cálido
    const colorB = new THREE.Color('#ffffff'); // Blanco puro
    const colorC = new THREE.Color('#ffaa44'); // Naranja suave

    for (let i = 0; i < neutrinoCount; i++) {
        // Distribución en todo el volumen, como un enjambre de luciérnagas
        ...
        // Fases aleatorias para movimiento orgánico
    ...
    const neutrinoSize = isMobile ? (isHighEndMobile ? 22.0 : 18.0) : 25.0;
    ...
    const neutrinoMat = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                vColor = color;
                // Titileo suave y orgánico de cada luciérnaga
                vAlpha = 0.35 + 0.65 * sin(uTime * 1.8 + position.x * 12.0 + position.y * 8.0); 
...
function tick() {
    if (currentWorld === 1) {
        ...
        interactiveStars.forEach(star => {
            ...
            star.getWorldPosition(vector); // Obtenemos la posición real ya rotada
